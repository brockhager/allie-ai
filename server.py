"""Top-level compatibility shim: re-export `backend.server` symbols.

This minimal shim keeps import-time side-effects low so tests that import
`server` can run without pulling in heavy model initialization at module
import time. The actual application lives in `backend/server.py`.
"""

from backend.server import *  # re-export for compatibility
"""Top-level compatibility shim so tests importing `server` can find the real backend.server module.

This file intentionally keeps import-time side-effects minimal and simply
re-exports symbols from `backend.server`. The real application lives in
`backend/server.py` and contains the heavy model and web server code; tests
should import this shim as `server` to remain compatible.
"""

from backend.server import *  # re-export backend symbols for tests
...
This file simply re-exports the backend.server symbols. It intentionally avoids
additional side-effects beyond importing the backend module (which the tests
already expect/mimic via mocks).
"""
